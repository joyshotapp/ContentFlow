...
def _collect_project_context(project_id: int, session) -> dict[str, Any]:
    ...
    business_goal_profile = _parse_business_goal_profile(project.business_goals if project else "")
...
async def run_strategic_agent(project_id: int) -> StrategicPlan:
    ...
    plan_result = _attach_action_controls(plan_result, context_snapshot)
...
async def execute_strategic_plan(plan_id: int) -> None:
    ...
    for action in sorted(actions, key=lambda a: a.get("priority", 99)):
        action = dict(action)
        action_type = action.get("action")
        can_execute, skip_reason = _can_execute_action(action)
        if not can_execute:
            skipped += 1
